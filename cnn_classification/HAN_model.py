...
class HierarchicalAttention:
    def __init__(self,  accusation_num_classes,learning_rate, batch_size, decay_steps, decay_rate, sequence_length,
                 num_sentences,vocab_size, embed_size,hidden_size, initializer=tf.random_normal_initializer(stddev=0.1),
                 clip_gradients=1.0,max_pooling_style='max_pooling', model='c_gru',num_filters=128,filter_sizes=[8],
                 stride_length=4,pooling_strategy='hier',hpcnn_filter_size=3,hpcnn_number_filters=32,num_repeat=4):
        ...
        if self.model=='gru':
            ...
            self.logits_accusation = self.inference_gru()  # [None, self.label_size]. main computation graph is here.
        elif self.model=='c_gru':
            print("going to use model:c_gru model")
            self.logits_accusation =self.inference_c_gru()
        elif self.model=='c_gru2':
            print("going to use model:c_gru2 model")
            self.logits_accusation =self.inference_c_gru2()
        elif self.model=='text_cnn':
            print("going to use model:text cnn model")
            self.logits_accusation =self.inference_text_cnn()
        elif self.model=='han':
            print("going to use model:hierarcial attention network")
            self.logits_accusation = self.inference_han()  # [None, self.label_size]. main computation graph is here.
        elif self.model=='dp_cnn':
            print("going to use model:deep pyramid cnn model.")
            self.logits_accusation = self.inference_deep_pyramid_cnn()  # [None, self.label_size]. main computation graph is here.
        else:
            print("going to use model: pooling")
            self.logits_accusation = self.inference_pooling()
    ...
    def attention(self,input_sequences,attention_level,reuse_flag=False):
        ...
        with tf.variable_scope("attention_" + str(attention_level),reuse=reuse_flag):
            ...
            weight=tf.reduce_sum(score,axis=2) #[batch_size,seq_length,1]
    ...
    def attention_multiply(self,input_sequences,attention_level,reuse_flag=False): #TODO need update
        ...
        with tf.variable_scope("attention_" + str(attention_level),reuse=reuse_flag):
            ...
            score=tf.expand_dims(tf.reduce_sum(score,axis=2),axis=-1) #/tf.sqrt(tf.cast(num_units,tf.float32)) #[batch_size,seq_length,1]
