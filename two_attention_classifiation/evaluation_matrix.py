...
def compute_MAP_MRR(y_logits_array, y_targetlabel_list):
    ...
    for k, y_logits_array_single in enumerate(y_logits_array):
        ...
        for i, (score, answer) in enumerate(sorted_answers, start=1):
            if answer in get_target_label_short(y_targetlabel_list[k]):
                if rank == 0:
                    rank = i
...
def compute_confuse_matrix_batch(y_targetlabel_list, y_logits_array, label_dict,name, accusa, vocabulary, fact):
    """
    ...
    """
    #file = open('data/bad_case.csv', 'w')

    acc = 0
    #k = 0
    for i, y_logits_list_single in enumerate(y_logits_array ):
        flag, label_dict, m, y_predict_labels = compute_confuse_matrix( y_targetlabel_list[i], y_logits_list_single, label_dict, name=name)

        #if flag  == False:
        #    print(' '.join([vocabulary[p] for p in fact[i]]).replace('_PAD', '').replace('\n', ''))
        #    print([accusa[j]  for j in get_target_label_short(y_targetlabel_list[i])])
        #    print([accusa[j]  for j in y_predict_labels])
        #    print()
        #    file.write(' '.join([vocabulary[p] for p in fact[i] ]).replace('_PAD', '') + '\n')
        #    file.write(str([accusa[j]  for j in get_target_label_short(y_targetlabel_list[i])])+'\n')
        #    file.write(str([accusa[j]  for j in y_predict_labels])+'\n')
        #    file.write('\n')
        #    k = k +1
...
def compute_confuse_matrix(y_targetlabel_list_single, y_logit_array_single, label_dict, name='default'):
    ...
    #y_predict_labels=[i for i in range(202) if y_logit_array_single[i]>=0.50] #TODO 0.5PW e.g.[2,12,13,10]
    ...
    return f, label_dict, acc, y_predict_labels 
