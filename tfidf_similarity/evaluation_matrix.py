...
def compute_confuse_matrix_batch(y_targetlabel_list, y_logits_array, label_dict,name='default'):
    ...
    for i, y_logits_array_single in enumerate(y_logits_array):
        label_dict, ac = compute_confuse_matrix(y_targetlabel_list[i], y_logits_array_single, label_dict, name=name)
        if ac  == True:
            acc = acc+1
    print('#####################  acc  ######################')
...
def compute_confuse_matrix(y_targetlabel_list_single, y_logit_array_single, label_dict, name='default'):
    ...
    # multi labels
    y_predict_labels=[i for i in range(len(y_logit_array_single)) if y_logit_array_single[i]>=0.1] #TODO 0.5PW e.g.[2,12,13,10]
