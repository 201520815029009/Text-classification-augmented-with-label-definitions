...
def load_data_multilabel(traning_data_path, valid_data_path, test_data_path, vocab_word2index, sentence_len,
    ...
    cache_file =cache_data_dir+"/"+'train_valid_test_article_32_article_des_word-113_shiwan_hightext.pik'  #train_valid_test_word
    ...
    train, _ = transform_data_to_index(train_lines, vocab_word2index, sentence_len, sentence_article, 'train', article, accusation_label2index)
    valid, valid_y = transform_data_to_index(valid_lines, vocab_word2index, sentence_len, sentence_article, 'valid', article, accusation_label2index)
    test, test_y = transform_data_to_index(test_lines, vocab_word2index, sentence_len, sentence_article, 'test', article, accusation_label2index)
    ...
    for i, line in enumerate(lines):
        ...
        if data_type == 'train':
            ...
            for item in accusation_list:
                input_article_pos = token_string_as_list(article[item])  # tokenize
                ...
                for _ in range(32):
                    input_article_neg = token_string_as_list(article[random.choice(article_remove)])  # tokenize
                    ...
                    datasample.append((fact, article_pos, article_neg))
            ...
            for i, (accuname, id) in enumerate(order):
                input_article_pos = token_string_as_list(article[accuname])  # tokenize
                ...
                datasample.append((fact, article_unknown))
    # shuffle
    data_x = []
    if data_type == 'train':
...
def create_or_load_vocabulary(predict_path, vocab_size, name_scope='cnn'):
    ...
    cache_path =cache_vocabulary_label_pik+"/"+'vocab_label_article_des_32_word_shiwan.pik'  #vocab_label
    ...
    if os.path.exists(cache_path):
        ...
        word_vocab_file = predict_path+"/"+'word_freq_shiwan_high.txt'
        ...
        accusation_freq_file = codecs.open(cache_vocabulary_label_pik+"/"+'accusation_freq_shiwan_high.txt',mode='a',encoding='utf-8')
