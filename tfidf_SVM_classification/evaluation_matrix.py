...
def compute_MAP_MRR(y_logits_array, y_targetlabel_list):
    ...
    for k, y_logits_array_single in enumerate(y_logits_array):
        rank = 0
        precisions = []
        sorted_answers = sorted(zip(y_logits_array_single, [m for m in range(0,202)]), key=lambda x: -x[0])

        for i, (value, answer) in enumerate(sorted_answers, start=1):
...
def compute_confuse_matrix_batch(y_targetlabel_list, y_logits_array, label_dict,name='default'):
    ...
    for i, y_logits_list_single in enumerate(y_logits_array ):
        label_dict, m = compute_confuse_matrix( y_targetlabel_list[i], y_logits_list_single, label_dict, name=name)
        if m  == True:
            acc = acc+1
    print('#####################  acc  ######################')
...
def compute_confuse_matrix(y_targetlabel_list_single, y_logit_array_single, label_dict, name='default'):
    ...
    #y_predict_labels =y_logit_array_single
    y_predict_labels=[i for i in range(len(y_logit_array_single)) if y_logit_array_single[i]==1] #TODO 0.5PW e.g.[2,12,13,10]
